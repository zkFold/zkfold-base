...
type Signature context =
    ...
    , Eq (Hash context)
    , Eq (Input context)
    , Eq (Address context, Datum context)
    , Eq (CurrencySymbol context, Token context)
    , Eq (Update context)
    , Eq (Value context)
    , Eq (List context (Hash context))
    , Eq (List context (Address context, Datum context))
    , Eq (List context (ContractId context, Token context))
    , Eq (List context (Input context))
    , Eq (Output context)
    , Eq (List context (Transaction context))
    , Eq (List context (List context (TransactionId context)))
