...
instance (SpecialSoundProtocol f a, Eq c, m ~ ProverMessage f a) => SpecialSoundProtocol f (CommitOpen m c a) where
...
opening :: forall f a c m . (SpecialSoundProtocol f a, Eq c, m ~ ProverMessage f a)
        => CommitOpen m c a
        -> Witness f a
        -> Input f a
