...
newtype WitnessMapPlonk = WitnessMap (Map.Map Integer F -> (PolyVec F Plonk, PolyVec F Plonk, PolyVec F Plonk))
-- TODO: make a proper implementation of Show
instance Show WitnessMapPlonk where
    show _ = "WitnessMap"
...
instance NonInteractiveProof Plonk where
    type Params Plonk       = ParamsPlonk
    type SetupSecret Plonk  = F
    type Setup Plonk        = (([F], [G1], G2, G2, F, F, F),
        ...
        (G1, G1, G1, G1, G1,
        G1, G1, G1),
        WitnessMapPlonk, (PolyVec F Plonk, PolyVec F Plonk, PolyVec F Plonk))
    type ProverSecret Plonk = ProverSecretPlonk
    type Witness Plonk      = WitnessInputPlonk
    type Input Plonk        = [F]
    ...
    setup (ParamsPlonk omega k1 k2 inputs ac) x =
        let wmap = acWitness $ mapVarArithmeticCircuit ac
            ...
            f i = case (i-1) `div` order @Plonk of
                0 -> omega^i
                1 -> k1 * (omega^i)
                2 -> k2 * (omega^i)
                _ -> error "setup: invalid index"
            s' = map f s
            sigma1 = toPolyVec $ take (order @Plonk) s'
            sigma2 = toPolyVec $ take (order @Plonk) $ drop (order @Plonk) s'
            sigma3 = toPolyVec $ take (order @Plonk) $ drop (2 * order @Plonk) s'
            ...
            qmE     = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega qm
            qlE     = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega ql
            qrE     = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega qr
            qoE     = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega qo
            qcE     = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega qc
            sigma1E = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega sigma1
            sigma2E = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega sigma2
            sigma3E = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega sigma3
    ...
    prove
        ...
        where
            n = order @Plonk
            zH = polyVecZero @F @Plonk @PlonkMaxPolyDegree

            (w1, w2, w3) = wmap wInput
            pubPoly = polyVecInLagrangeBasis omega $ toPolyVec @F @Plonk wPub
            ...
            z  = polyVecQuadratic b9 b8 b7 * zH + polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega gp
            ...
            t3  = (a + scalePV beta sigma1 + scalePV gamma one)
                ...
                * zo
            t4 = (z - one) * polyVecLagrange @F @Plonk @PlonkMaxPolyDegree 1 omega
            ...
            t_lo   = t_lo' + scalePV b10 (polyVecZero @F @Plonk @PlonkMaxPolyDegree + one)
            t_mid  = t_mid' + scalePV b11 (polyVecZero @F @Plonk @PlonkMaxPolyDegree + one) - scalePV b10 one
            ...
            lagrange1_xi = polyVecLagrange @F @Plonk @PlonkMaxPolyDegree 1 omega `evalPolyVec` xi
    ...
    verify
        ...
        where
            ...
            zH_xi        = polyVecZero @F @Plonk @PlonkMaxPolyDegree `evalPolyVec` xi
            lagrange1_xi = polyVecLagrange @F @Plonk @PlonkMaxPolyDegree 1 omega `evalPolyVec` xi
            pubPoly_xi   = polyVecInLagrangeBasis @F @Plonk @PlonkMaxPolyDegree omega (toPolyVec ws) `evalPolyVec` xi
