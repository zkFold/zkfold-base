...
--
class AlgorithmSetup (algorithm :: Symbol) (backend :: Natural -> Type -> Type) a where
    ...
    initialHashes :: V.Vector (ByteString (WordSize algorithm) backend a)
    -- ^ Initial hash values which will be mixed with the message bits.

    roundConstants :: V.Vector (ByteString (WordSize algorithm) backend a)
    -- ^ Constants used in the internal loop, one per each round.

    truncateResult :: ByteString (8 * WordSize algorithm) backend a -> ByteString (ResultSize algorithm) backend a
...
instance (FromConstant Natural (ByteString 32 b a)) => AlgorithmSetup "SHA256" b a where
...
instance (FromConstant Natural (ByteString 32 b a), Truncate (ByteString 256 b a) (ByteString 224 b a)) => AlgorithmSetup "SHA224" b a where
...
instance (FromConstant Natural (ByteString 64 b a)) => AlgorithmSetup "SHA512" b a where
...
instance (FromConstant Natural (ByteString 64 b a), Truncate (ByteString 512 b a) (ByteString 384 b a)) => AlgorithmSetup "SHA384" b a where
...
instance (FromConstant Natural (ByteString 64 b a), Truncate (ByteString 512 b a) (ByteString 224 b a)) => AlgorithmSetup "SHA512/224" b a where
...
instance (FromConstant Natural (ByteString 64 b a), Truncate (ByteString 512 b a) (ByteString 256 b a)) => AlgorithmSetup "SHA512/256" b a where
...
--
type SHA2 algorithm backend element k =
   ( AlgorithmSetup algorithm backend element
   , KnownNat k
   , Finite element
   , NFData element
   , NFData (ByteString (WordSize algorithm) backend element)
   , FromConstant Natural element
   , FromConstant Natural (ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element)
   , KnownNat (ChunkSize algorithm)
   , KnownNat (WordSize algorithm)
   , KnownNat (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm))
   , Iso (UInt (WordSize algorithm) backend element) (ByteString (WordSize algorithm) backend element)
   , Iso (ByteString (WordSize algorithm) backend element) (UInt (WordSize algorithm) backend element)
   , AdditiveSemigroup (UInt (WordSize algorithm) backend element)
   , BoolType (ByteString (WordSize algorithm) backend element)
   , ShiftBits (ByteString (WordSize algorithm) backend element)
   , ShiftBits (ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element)
   , BoolType (ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element)
   , Extend (ByteString k backend element) (ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element)
   , ToWords (ByteString (ChunkSize algorithm) backend element) (ByteString (WordSize algorithm) backend element)
   , Concat (ByteString (WordSize algorithm) backend element) (ByteString (8 * WordSize algorithm) backend element)
   , ToWords (ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element) (ByteString (ChunkSize algorithm) backend element)
   )
...
--
sha2
    :: forall (algorithm :: Symbol) backend element k
    .  SHA2 algorithm backend element k
    => ByteString k backend element -> ByteString (ResultSize algorithm) backend element
sha2 messageBits = sha2Blocks @algorithm @backend @element chunks
    where
        paddedMessage :: ByteString (PaddedLength k (ChunkSize algorithm) (2 * WordSize algorithm)) backend element
        paddedMessage = sha2Pad @(ChunkSize algorithm) @(2 * WordSize algorithm) messageBits

        chunks :: [ByteString (ChunkSize algorithm) backend element]
        chunks = toWords paddedMessage

-- | Pad the input bytestring according to the rules described in @PaddedLength@
--
sha2Pad
    :: forall (padTo :: Natural) (lenBits :: Natural) backend (k :: Natural) element
    .  KnownNat k
    => KnownNat (PaddedLength k padTo lenBits)
    => FromConstant Natural (ByteString (PaddedLength k padTo lenBits) backend element)
    => ShiftBits (ByteString (PaddedLength k padTo lenBits) backend element)
    => BoolType (ByteString (PaddedLength k padTo lenBits) backend element)
    => Extend (ByteString k backend element) (ByteString (PaddedLength k padTo lenBits) backend element)
    => ByteString k backend element
    -> ByteString (PaddedLength k padTo lenBits) backend element
sha2Pad bs = grown || fromConstant padValue
    where
        ...
        grown :: ByteString (PaddedLength k padTo lenBits) backend element
...
--
instance (KnownNat n, FromConstant Natural (ByteString n b a)) => ToWords Natural (ByteString n b a) where
    toWords = P.reverse . toWords'
        where
            toWords' :: Natural -> [ByteString n b a]
            toWords' 0 = []
...
--
type SHA2N algorithm backend element =
   ( AlgorithmSetup algorithm backend element
   , Finite element
   , NFData element
   , NFData (ByteString (WordSize algorithm) backend element)
   , FromConstant Natural element
   , FromConstant Natural (ByteString (ChunkSize algorithm) backend element)
   , KnownNat (ChunkSize algorithm)
   , KnownNat (WordSize algorithm)
   , Iso (UInt (WordSize algorithm) backend element) (ByteString (WordSize algorithm) backend element)
   , Iso (ByteString (WordSize algorithm) backend element) (UInt (WordSize algorithm) backend element)
...
--
sha2Natural
    :: forall (algorithm :: Symbol) (backend :: Natural -> Type -> Type) element
    .  SHA2N algorithm backend element
    => Natural -> Natural -> ByteString (ResultSize algorithm) backend element
sha2Natural numBits messageBits = sha2Blocks @algorithm @backend @element chunks
    where
...
--
sha2Blocks
    :: forall algorithm (backend :: Natural -> Type -> Type) element
    .  AlgorithmSetup algorithm backend element
    => NFData (ByteString (WordSize algorithm) backend element)
    => Iso (ByteString (WordSize algorithm) backend element) (UInt (WordSize algorithm) backend element)
    => AdditiveSemigroup (UInt (WordSize algorithm) backend element)
    ...
    => ToWords (ByteString (ChunkSize algorithm) backend element) (ByteString (WordSize algorithm) backend element)
    => Concat (ByteString (WordSize algorithm) backend element) (ByteString (8 * WordSize algorithm) backend element)
    => [ByteString (ChunkSize algorithm) backend element] -> ByteString (ResultSize algorithm) backend element
sha2Blocks chunks = truncateResult @algorithm @backend @element $ concat $ V.toList hashParts
    where
        rounds :: Int
        rounds = V.length $ roundConstants @algorithm @backend @element

        hashParts :: V.Vector (ByteString (WordSize algorithm) backend element)
        hashParts = V.create $ do
            !hn <- V.thaw $ initialHashes @algorithm @backend @element

            forM_ chunks $ \chunk -> do
                let words = toWords @(ByteString (ChunkSize algorithm) backend element) @(ByteString (WordSize algorithm) backend element) chunk
                messageSchedule <- VM.unsafeNew @_ @(ByteString (WordSize algorithm) backend element) rounds
                ...
                forM_ [16 .. rounds P.- 1] $ \ix -> do
                    ...
                    let (sh0, sh1, sh2, sh3, sh4, sh5) = sigmaShifts @algorithm @backend @element
                    ...
                    VM.write messageSchedule ix $! from (from w16 + from s0 + from w7 + from s1 :: UInt (WordSize algorithm) backend element)
                ...
                --forM_ [0 .. 2] $ \ix -> do
                    ...
                    let ki = roundConstants @algorithm @backend @element V.! ix
                    wi <- messageSchedule `VM.read` ix

                    let (sh0, sh1, sh2, sh3, sh4, sh5) = sumShifts @algorithm @backend @element
                        s1    = force $ (e `rotateBitsR` sh3) `xor` (e `rotateBitsR` sh4) `xor` (e `rotateBitsR` sh5)
                        ch    = force $ (e && f) `xor` (not e && g)
                        temp1 = force $ from (from h + from s1 + from ch + from ki + from wi :: UInt (WordSize algorithm) backend element) :: ByteString (WordSize algorithm) backend element
                        s0    = force $ (a `rotateBitsR` sh0) `xor` (a `rotateBitsR` sh1) `xor` (a `rotateBitsR` sh2)
                        maj   = force $ (a && b) `xor` (a && c) `xor` (b && c)
                        temp2 = force $ from (from s0 + from maj :: UInt (WordSize algorithm) backend element) :: ByteString (WordSize algorithm) backend element

                    ST.writeSTRef hRef g
                    ST.writeSTRef gRef f
                    ST.writeSTRef fRef e
                    ST.writeSTRef eRef $ from (from d + from temp1 :: UInt (WordSize algorithm) backend element)
                    ST.writeSTRef dRef c
                    ST.writeSTRef cRef b
                    ST.writeSTRef bRef a
                    ST.writeSTRef aRef $ from (from temp1 + from temp2 :: UInt (WordSize algorithm) backend element)
                ...
                VM.modify hn (\w -> from (from w + from a :: UInt (WordSize algorithm) backend element)) 0
                VM.modify hn (\w -> from (from w + from b :: UInt (WordSize algorithm) backend element)) 1
                VM.modify hn (\w -> from (from w + from c :: UInt (WordSize algorithm) backend element)) 2
                VM.modify hn (\w -> from (from w + from d :: UInt (WordSize algorithm) backend element)) 3
                VM.modify hn (\w -> from (from w + from e :: UInt (WordSize algorithm) backend element)) 4
                VM.modify hn (\w -> from (from w + from f :: UInt (WordSize algorithm) backend element)) 5
                VM.modify hn (\w -> from (from w + from g :: UInt (WordSize algorithm) backend element)) 6
                VM.modify hn (\w -> from (from w + from h :: UInt (WordSize algorithm) backend element)) 7
