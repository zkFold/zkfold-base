...
addSPolyStep :: (Eq c, FiniteField c, Ord a, AdditiveGroup a, MultiplicativeMonoid a) =>
...
groebnerStep :: (Eq c, FiniteField c, Ord a, AdditiveGroup a, MultiplicativeMonoid a) =>
                Polynom c a -> [Polynom c a] -> (Polynom c a, [Polynom c a])
groebnerStep h fs
    | zeroP h   = (h, fs)
    | otherwise =
