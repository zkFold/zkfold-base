...
instance (Finite (Zp p)) => EllipticCurve (Ed25519 UInt (Zp p)) where
    type BaseField (Ed25519 UInt (Zp p)) = UInt 256 Vector (Zp p)
    type ScalarField (Ed25519 UInt (Zp p)) = UInt 256 Vector (Zp p)
...
instance
    ...
    restore c o = bool @(Bool (ArithmeticCircuit 1 a)) @(Point (Ed25519 ArithmeticCircuit a)) (Point x y) Inf ((x == zero) && (y == zero))
        where
            (piecesX, piecesY) = V.splitAt @(TypeSize a (UInt 256 ArithmeticCircuit a)) o
...
instance
    ...
    add x y = bool @(Bool (ArithmeticCircuit 1 a)) @(Point (Ed25519 ArithmeticCircuit a)) (acAdd25519 x y) (acDouble25519 x) (x == y)
    ...
    --
    mul sc x = sum $ P.zipWith (\b p -> bool @(Bool (ArithmeticCircuit 1 a)) zero p (isSet bits b)) [255, 254 .. 0] (P.iterate (\e -> e + e) x)
        where
