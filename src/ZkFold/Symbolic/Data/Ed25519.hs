...
instance (Finite (Zp p)) => EllipticCurve (Ed25519 UInt (Zp p)) where
    type BaseField (Ed25519 UInt (Zp p)) = UInt 256 (Interpreter (Zp p))
    type ScalarField (Ed25519 UInt (Zp p)) = UInt 256 (Interpreter (Zp p))
...
instance
    ...
    type TypeSize a (Point (Ed25519 ArithmeticCircuit a)) =
        TypeSize a (UInt 256 (ArithmeticCircuit a)) + TypeSize a (UInt 256 (ArithmeticCircuit a))
