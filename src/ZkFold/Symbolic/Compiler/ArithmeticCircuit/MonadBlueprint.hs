...
instance (Arithmetic a, Ord (Rep i), Representable i) => MonadBlueprint i (Var i) a (State (ArithmeticCircuit a i U1)) where
    runCircuit r = modify (<> r {acOutput = U1}) $> acOutput r

circuit :: (Arithmetic a, Ord (Rep i), Representable i) => (forall v m . MonadBlueprint i v a m => m v) -> ArithmeticCircuit a i Par1
...
circuitF :: forall a i o . (Arithmetic a, Ord (Rep i), Representable i) => (forall v m . MonadBlueprint i v a m => m (o v)) -> ArithmeticCircuit a i o
...
circuits :: forall a i o . (Arithmetic a, Functor o, Ord (Rep i), Representable i) => (forall v m . MonadBlueprint i v a m => m (o v)) -> o (ArithmeticCircuit a i Par1)
