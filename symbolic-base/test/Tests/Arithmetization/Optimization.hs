...
testBool :: forall a . (Arithmetic a, Binary a, Typeable a) => ArithmeticCircuit a (U1 :*: U1) (Par1 :*: U1) Par1
testBool = compile @a identBool
    where
        identBool :: Bool c -> Bool c
        identBool x = x

testConst :: forall a . (Arithmetic a, Binary a, Typeable a) => ArithmeticCircuit a U1 Par1 Par1
...
testLinVar :: forall a . (Arithmetic a, Binary a, Typeable a) => ArithmeticCircuit a U1 Par1 Par1
testLinVar = fromCircuitF idCircuit $ \(Par1 i0) -> do
    i1 <- newAssigned (($ i0) + one)
    i2 <- newAssigned (($ i1) * ($ i0))
    return (Par1 i2)

specOptimization' :: forall a . (Arithmetic a, Binary a, Show a, Typeable a) => Spec
specOptimization' = do
...
specOptimization :: forall a . (Arithmetic a, Binary a, Show a, Typeable a) => IO ()
