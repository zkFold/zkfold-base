...
--     , Conditional (Bool ctx) (Accumulator k i c f)
--     -- , Traversable (NARKInstanceProof 1 i c)
--     )
--     => Predicate i p ctx'
--     -> (f -> f')
--     -> (w' -> w)
...
--     let
--         protocol :: FiatShamir k i p c ctx
--         protocol = fiatShamir @algo $ commitOpen $ specialSoundProtocol @d phi

--         narkIP' :: NARKInstanceProof k i c ctx
...
--         accScheme :: AccumulatorScheme d k i c ctx
